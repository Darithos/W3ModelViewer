...
| `ratescan.py` | `python ratescan.py <dir or files> [--verbose] [--examples=N]` | Every PAR_'s `emit_rate` and `emit_count` anim header, the STC collection each track lives in (per sequence), its key frames and values, plus corpus statistics. `--verbose` on one of our exports shows at a glance which sequences carry the track — this is what exposed the synthesised Stand copy carrying no float tracks. |
| `countmax.py` | `python countmax.py <dir>` | `emit_max` against each `emit_count` burst's peak and hold length over a corpus. Proved the count is edge-triggered (median hold one 33 ms key; a 2 s hold of 15 caps at 31). |
